...
**Authors:** Ilya Sutskever Google ilyasu@google.com &Oriol Vinyals Google vinyals@google.com &Quoc V. Le Google qvl@google.com
...
Short-Term Memory (LSTM) architecture [16] can solve
...
(fig. 1). The second LSTM is essentially a
...
approach is closely related to Kalchbrenner and Blunsom [18] who were
the first to map the entire input sentence to vector, and is related
to Cho et al. [5] although the latter was used only for rescoring hypotheses
produced by a phrase-based system. Graves
[10] introduced a novel differentiable attention mechanism
...
between the inputs and the outputs [11].
...
lists of the SMT baseline on the same task [29]. By
...
result on this task (which is 37.0 [9]).
...
[26]. We were able to do well on long sentences because we
...
simpler (see sec. 2 and 3.3). As a result, SGD could learn
...
The Recurrent Neural Network (RNN) [31, 28]
is a natural generalization of feedforward neural networks to
sequences. Given a sequence of inputs $(x_{1},\ldots,x_{T})$, a
standard RNN computes a sequence of outputs $(y_{1},\ldots,y_{T})$ by
...
taken by Cho et al. [5]). While it could work
...
(figure 1)
[14, 4, 16, 15]. However, the Long
Short-Term Memory (LSTM) [16] is known to learn
...
$p(y_{1},\ldots,y_{T^{\prime}}|x_{1},\ldots,x_{T})$ where $(x_{1},\ldots,x_{T})$ is an
input sequence and $y_{1},\ldots,y_{T^{\prime}}$ is its corresponding output
sequence whose length $T^{\prime}$ may differ from $T$. The LSTM computes this
conditional probability by first obtaining the fixed-dimensional
representation $v$ of the input sequence $(x_{1},\ldots,x_{T})$ given by
the last hidden state of the LSTM, and then computing the probability
of $y_{1},\ldots,y_{T^{\prime}}$ with a standard LSTM-LM formulation whose
initial hidden state is set to the representation $v$ of
$x_{1},\ldots,x_{T}$:

In this equation, each $p(y_{t}|v,y_{1},\ldots,y_{t-1})$ distribution
is represented with a softmax over all the words in the
vocabulary. We use the LSTM formulation from Graves [10].
...
scheme is outlined in figure 1, where the
...
[18]. Second, we found that deep LSTMs significantly
...
asked to map $c,b,a$ to $\alpha,\beta,\gamma$, where $\alpha,\beta,\gamma$ is the translation of $a,b,c$. This way, $a$ is in close
...
[29]. We chose this translation task and this specific
...
where $\mathcal{S}$ is the training set. Once training is complete, we produce
...
baseline system [29]. To rescore an n-best list, we
...
the problem has a large “minimal time lag” [17]. By reversing the
...
sec. 3.7), which suggests that reversing the
...
[10, 25] by scaling it when its norm exceeded
a threshold. For each training batch, we compute $s=\left\|g\right\|_{2}$, where $g$ is the gradient divided by 128. If $s>5$, we set
$g=\frac{5g}{s}$.
...
We used the cased BLEU score [24] to evaluate the quality of our
translations. We computed our BLEU scores using multi-bleu.pl11There several variants of the BLEU score, and each variant is defined with a perl script.
on the tokenized predictions and ground truth.
This way of evaluating the BELU score is consistent with [5] and [2], and reproduces
the 33.3 score of [29].
However, if we evaluate the best WMT’14 system [9]
(whose predictions can be downloaded from statmt.org\matrix) in this manner, we get
37.0, which is greater than the 35.8 reported by statmt.org\matrix.

The results are presented in tables 1 and
2. Our best results are obtained with an
...
sentences, which is shown quantitatively in figure 3.
Table 3 presents several examples of long sentences and
their translations.
...
Figure 2 visualizes some of the learned
...
applying an RNN-Language Model (RNNLM) [23] or
a Feedforward Neural Network Language Model (NNLM) [3] to an
MT task is by rescoring the n-best lists of a strong MT baseline
[22], which reliably improves translation quality.
...
work include Auli et al. [1], who combine an NNLM with a
topic model of the input sentence, which improves rescoring
performance. Devlin et al. [8] followed a similar
...
Our work is closely related to Kalchbrenner and Blunsom [18],
...
system. Bahdanau et al. [2] also attempted direct
...
al. [5] and achieved encouraging results. Likewise,
Pouget-Abadie et al. [26] attempted to address the memory
problem of Cho et al. [5] by translating pieces of the source
...
al. [12], whose model represents the inputs and outputs by
...
fig. 1), we believe that a standard RNN
...
similar to ours [5, 2, 26]. And yet,
